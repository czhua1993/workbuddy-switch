...
pub fn slim_sessions(
    ...
    // 按账号真实档位定位 workbuddy.db（pr-62 原版硬编码 Cn，会误清/漏清国际版账号）。
    let variant = account::load_accounts()
        .into_iter()
        .find(|a| a.get("uid").and_then(|v| v.as_str()) == Some(uid))
        .map(|a| crate::modules::variant::WbVariant::from_account(&a))
        .unwrap_or(crate::modules::variant::WbVariant::Cn);
    let mut db_backup: Option<String> = None;
    if !dry_run {
        let root = backup_dir().join("projects_anchor").join(backup_stamp());
        db_backup = backup_workbuddy_db(&SessionPaths::for_variant(variant), &root)
            .ok()
